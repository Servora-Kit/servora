...
// QueryOwnedTenants chains the current query on the "owned_tenants" edge.
func (_q *UserQuery) QueryOwnedTenants() *TenantQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOwnedTenants:   _q.withOwnedTenants.Clone(),
...
// WithOwnedTenants tells the query-builder to eager-load the nodes that are connected to
// the "owned_tenants" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOwnedTenants(opts ...func(*TenantQuery)) *UserQuery {
	query := (&TenantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOwnedTenants = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOwnedTenants; query != nil {
		if err := _q.loadOwnedTenants(ctx, query, nodes,
			func(n *User) { n.Edges.OwnedTenants = []*Tenant{} },
			func(n *User, e *Tenant) { n.Edges.OwnedTenants = append(n.Edges.OwnedTenants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadOwnedTenants(ctx context.Context, query *TenantQuery, nodes []*User, init func(*User), assign func(*User, *Tenant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenant.FieldOwnerUserID)
	}
	query.Where(predicate.Tenant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OwnedTenantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
