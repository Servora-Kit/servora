...
func wireApp(confServer *conf.Server, discovery *conf.Discovery, confRegistry *conf.Registry, confData *conf.Data, app *conf.App, trace *conf.Trace, metrics *conf.Metrics, mail *conf.Mail, confBiz *conf2.Biz, svcIdentity bootstrap.SvcIdentity, logger log.Logger) (*kratos.App, func(), error) {
	...
	userUsecase := biz.NewUserUsecase(userRepo, logger, app, authnRepo, authnUsecase, organizationRepo, tenantRepo, authZRepo)
	...
	rbacRoleRepo := data.NewRbacRoleRepo(dataData, logger)
	rbacPermissionRepo := data.NewRbacPermissionRepo(dataData, logger)
	rbacMenuRepo := data.NewRbacMenuRepo(dataData, logger)
	rbacUsecase := biz.NewRbacUsecase(rbacRoleRepo, rbacPermissionRepo, rbacMenuRepo, logger)
	rbacService := service.NewRbacService(rbacUsecase, userUsecase, tenantUsecase, organizationUsecase)
	...
	dictService := service.NewDictService(dictUsecase, userUsecase)
	...
	httpServer := server.NewHTTPServer(confServer, app, httpMiddleware, telemetryMetrics, logger, handler, authnService, userService, testService, organizationService, tenantService, applicationService, rbacService, positionService, dictService, capCap, provider, loginHandler, loginCompleteHandler)
