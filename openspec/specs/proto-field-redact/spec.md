# Spec: proto-field-redact

## Purpose

Defines requirements for proto field-level log redaction via `protoc-gen-redact`. Generated `Redact()` methods are consumed by Kratos `logging.Server` middleware to mask sensitive fields in logs.
