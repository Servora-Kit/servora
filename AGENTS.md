...
Servora 是 ProtoBuf-contract-first 的模块化 Go 框架库。仓库同时包含运行时库、protoc 插件、CLI、公共 proto 与生成代码模块。
...
| `obs/` | logging/telemetry/audit 可观测性能力 |
| `infra/` | broker/db/k8s/redis 等基础设施适配 |
...
提交格式：`type(scope): description`。type：`feat`/`fix`/`refactor`/`docs`/`test`/`chore`。scope 建议使用一级域或二级域：`api`、`cmd`、`core/bootstrap`、`transport/server`、`security/authn`、`obs/audit`、`infra/redis` 等。
...
- `import` 相对于 `api/protos/`，例如 `import "servora/audit/v1/audit.proto";`。
...
4. 执行 `go build ./...` 或更窄的相关测试。
5. 打主 tag；如有 proto/gen 产物变化，执行 `make tag.api TAG=v0.x.y`。
6. 执行 `make bsr.push`。
...
- transport TLS 构造归 `security/tls`，不要把证书解析逻辑散落到 client/server 子包。
