...
| `extra/<域>/v1/` | Per-domain optional configuration. Each domain lives in its own namespace and proto file, loaded via `bootstrap.Scan`. | `audit` / `cors` / `mail` / `jwt` |
| `infra/<域>/v1/` | Infrastructure optional sections consumed by infra runtime packages. | `redis` / `kafka` / `db/clickhouse` |
